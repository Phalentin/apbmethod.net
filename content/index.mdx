...
<Steps>
{<h3>Step 1: 2x2x3 block on the left side</h3>}
...
{<h3>Step 2: Solving the back-right F2L-pair</h3>}
The back-right F2L pair is solved intuitively.

{<h3>Step 3: Solve the edge-orientation of the remaining edges (EO)</h3>}
The edge-orientation of the remaining edges is solved algorithmically (11 algorithms).

{<h3> Step 4: Solve the last three F2L pieces (LXS)</h3>}
The remaining three F2L pieces are solved algorithmically (116 algorithms).

{<h3>Step 5: Solve the last layer using ZBLL</h3>}
